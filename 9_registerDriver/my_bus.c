...
static int my_bus_match(struct device *dev, struct device_driver *drv)
{
    ...
    /* 严格匹配：设备名和驱动名必须完全相等 */
    if (strcmp(dev->kobj.name, drv->name) == 0) {
        pr_info("设备 '%s' 与驱动 '%s' 严格匹配成功\n", 
                dev->kobj.name, drv->name);
        return 1;  /* 匹配成功 */
    }
...
// 方法2
/* bus_attribute 是Linux内核中定义的标准结构体
 * 用于描述总线的属性文件
 * 包含:
 * - struct attribute attr: 基本属性(名称、权限等)
 * - show: 读取属性时的回调函数
 * - store: 写入属性时的回调函数
 * 
 * 在sysfs中会显示为 /sys/bus/<bus_name>/<attr_name> 文件
 */

static struct bus_attribute bus_attr_bus = {
    .attr = { 
        .name = "bus", 
        .mode = 0444 /* 只读权限 */
    },
    .show = bus_show,
    .store = NULL /* 没有store函数，表示不可写 */
};
